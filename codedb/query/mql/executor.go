...
import (
	...
	"github.com/scrypster/muninndb/codedb/query"
...
func buildFilter(f *query.Filter, pred Predicate) error {
	switch p := pred.(type) {
	case *StatePredicate:
		state, err := engine.ParseLifecycleState(p.State)
		if err != nil {
...
func convertFilterToMBPFilters(f *query.Filter) []mbp.Filter {
	...
	// State filters — use engine.LifecycleState directly so passesMetaFilter's
	// type assertion (f.Value.(engine.LifecycleState)) succeeds.
