...
func TestFilter_States_OR(t *testing.T) {
	f := &Filter{
		States: []engine.LifecycleState{engine.StateActive, engine.StatePaused},
	}

	engrams := []*engine.Engram{
		{ID: engine.NewULID(), Concept: "active", State: engine.StateActive},
		{ID: engine.NewULID(), Concept: "paused", State: engine.StatePaused},
		{ID: engine.NewULID(), Concept: "archived", State: engine.StateArchived},
	}
...
func TestFilter_MinRelevance(t *testing.T) {
	...
	engrams := []*engine.Engram{
		{ID: engine.NewULID(), Concept: "high", Relevance: 0.8, State: engine.StateActive},
		{ID: engine.NewULID(), Concept: "medium", Relevance: 0.5, State: engine.StateActive},
		{ID: engine.NewULID(), Concept: "low", Relevance: 0.3, State: engine.StateActive},
	}
...
func TestFilter_MinConfidence(t *testing.T) {
	...
	engrams := []*engine.Engram{
		{ID: engine.NewULID(), Concept: "high", Confidence: 0.9, State: engine.StateActive},
		{ID: engine.NewULID(), Concept: "medium", Confidence: 0.6, State: engine.StateActive},
	}
...
func TestFilter_Combined(t *testing.T) {
	...
	f := &Filter{
		...
		States:        []engine.LifecycleState{engine.StateActive},
...
func TestFilter_Apply_Pagination(t *testing.T) {
	engrams := make([]*engine.Engram, 0, 10)
	for range 10 {
		engrams = append(engrams, &engine.Engram{
			ID:    engine.NewULID(),
			State: engine.StateActive,
		})
	}
...
func TestFilter_Creator(t *testing.T) {
	...
	engrams := []*engine.Engram{
		{ID: engine.NewULID(), CreatedBy: "alice", State: engine.StateActive},
		{ID: engine.NewULID(), CreatedBy: "bob", State: engine.StateActive},
		{ID: engine.NewULID(), CreatedBy: "alice", State: engine.StateActive},
	}
...
func TestFilter_MinStability(t *testing.T) {
	...
	engrams := []*engine.Engram{
		{ID: engine.NewULID(), Stability: 15, State: engine.StateActive},
		{ID: engine.NewULID(), Stability: 10, State: engine.StateActive},
		{ID: engine.NewULID(), Stability: 5, State: engine.StateActive},
	}
...
func TestFilter_UpdatedAfter(t *testing.T) {
	...
	engrams := []*engine.Engram{
		{ID: engine.NewULID(), UpdatedAt: now, State: engine.StateActive},
		{ID: engine.NewULID(), UpdatedAt: now.Add(-24 * time.Hour), State: engine.StateActive},
	}
...
func TestFilter_EmptyVault(t *testing.T) {
	...
	engrams := []*engine.Engram{
		nil,
		{ID: engine.NewULID(), State: engine.StateActive},
		nil,
		{ID: engine.NewULID(), State: engine.StateActive},
	}
...
func TestFilter_TagsEmptySlice(t *testing.T) {
	...
	engrams := []*engine.Engram{
		{ID: engine.NewULID(), Tags: []string{}, State: engine.StateActive},
		{ID: engine.NewULID(), Tags: []string{"tag1"}, State: engine.StateActive},
	}
...
func TestFilterApply_LimitDefault(t *testing.T) {
	...
	engrams := make([]*engine.Engram, 25)
	for i := range engrams {
		engrams[i] = &engine.Engram{Concept: "item"}
	}
