...
func (f *Filter) Match(e *engine.Engram) bool {
	...
	if len(f.States) > 0 {
		if !slices.Contains(f.States, e.State) {
			return false
		}
	}
...
func (f *Filter) Apply(engrams []*engine.Engram) []*engine.Engram {
	// First, filter all engrams with Match
	filtered := make([]*engine.Engram, 0, len(engrams))
	...
	if start >= len(filtered) {
		return []*engine.Engram{} // empty slice
	}
